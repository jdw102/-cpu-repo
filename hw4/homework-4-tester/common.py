...
# **Attempt** to find the lack of a recursive function use in C programs
# Very possible to fool but more effort required than writing a recursive program to begin with
def no_recursive_function_used(suite_name):
    filename = "%s.c" % suite_name
    myFile = open(filename,"r")
    str1 = myFile.read() # slurp whole file
    myFile.close()
    str1 = re.sub('\".*\"'," ",str1) # eliminate quoted strings
    str1 = re.sub('//.*'," ",str1)  # eliminate one-line comments
    regex = r"\s+([a-zA-Z0-9_]+?)\s*?\([^;]*?\)\s*?\{.*?\1\s*?\([^;]*?\).*?;.*?\}"
    matches = re.findall(regex, str1, re.MULTILINE | re.DOTALL)
...
class Grader():
    ...
    """
    def __init__(self, test_suite, settings_file, make_expected=False, mode="", extra_credit_multiplier=1.0):
        self.test_suite = test_suite
        self.make_expected = make_expected
        self.extra_credit_multiplier = extra_credit_multiplier
        ...
        self.mode = mode if mode != "" else setting["mode"]
        assert self.mode in VALID_TEST_MODES, "Invalid test mode."
        
        if self.mode == "logisim" or self.mode == "java":
            # find java
            self.java = find_java()
        ...
        self.non_recursive_penalty = setting.get("non_recursive_penalty", [])
    ...
    def run(self):
        ...
        if self.mode_overridden:
            print((TextColors.GREEN + "Mode overridden to %s" + TextColors.END) % self.mode)
        ...
        return test_outputs, total_score * self.extra_credit_multiplier
    ...
    def run_test_suite(self, test_suite_name, test_outputs, total_score):
        ...
        if test_suite_name in self.non_recursive_penalty:
            if no_recursive_function_used(test_suite_name):
                print(TextColors.RED + "*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*" + TextColors.END)
                print(TextColors.RED + "* %s: NO RECURSIVE FUNCTION DETECTED IN SOURCE CODE - PENALTY APPLIED!" % test_suite_name + TextColors.END)
                print(TextColors.RED + "* All test scores will be multiplied by 0 if a human confirms this!" + TextColors.END)
                print(TextColors.RED + "*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*" + TextColors.END)
                # self.will_penalize[test_suite_name] *= 0
    ...
    def execute_test(self, test_suite_name, test_num, args, is_valgrind, diff_type, filter_type, timeout=None):
        ...
        if mode == "exe":
            ...
            command = self.java 
            arguments = [executable_file_name] + args
        elif mode == "spim":
            command = "spim"
